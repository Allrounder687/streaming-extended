...
(function () {
  ...
  // IMDB Ratings Manager Class
  class IMDBRatingsManager {
    constructor() {
    ...
    init() {
      ...
      chrome.storage.sync.get(['showIMDBRatings'], (result) => {
        this.enabled = result.showIMDBRatings !== false;
        if (this.enabled) {
    ...
    startTitleMonitoring() {
      // Monitor for title changes every 3 seconds
      setInterval(() => {
        if (this.enabled) {
      ...
      setTimeout(() => this.checkForTitleChange(), 2000);
    }
  ...
  try {
    ...
    setTimeout(() => {
      try {
        // Initialize IMDB ratings directly
        window.imdbRatings = new IMDBRatingsManager();
